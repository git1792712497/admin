<template>
  <section ref="containerTarget" :style="{height: `${containerHeight}px` }" class="relative">
    <!-- 因为列数不确定，所以需要根据列数计算每列的宽度，所以等待列宽计算完成，并且有了数据源之后进行渲染 -->
    <template v-if="columnWidth && data.length">
      <!-- 通过动态的 style 来去计算对应的列宽、left、top -->
      <div v-for="(item, index) in data" :key="key ? item[key] : index" :style="{width: `${columnWidth}px`,left:`${item._style?.left}px`,top:`${item._style?.top}px`}" class="m-waterfall-item absolute duration-300">
        <slot :index="index" :item="item" :width="columnWidth"/>
      </div>
    </template>
     <div v-else>加载中...</div>
  </section>
