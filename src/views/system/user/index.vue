<template>
  <el-card>
    <template #header><TopSearch @addRole="AddUserDialogRef.openDialog()"/></template>
    ...
    <el-table v-loading="loading" size="large" border :data="data?.data" row-key="id">
      ...
      <el-table-column fixed="right" label="操作" width="200" align="center">
        <template #default="{row}">
          <el-button link type="primary" size="small" @click="AddUserDialogRef.openDialog(row)">编辑 <el-icon><Edit /></el-icon></el-button>
          <el-popconfirm title="是否确认删除?" @confirm="handleDelete(row.id)">
            <template #reference>
              <el-button v-auth="['deleteUser']" link type="danger" size="small">删除<el-icon><Delete /></el-icon></el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
